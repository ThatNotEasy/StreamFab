...
from text.Mp4VttParser import Mp4VttParser
from text.Mp4TtmlParser import Mp4TtmlParser
...
def transVtt(infile , inpath):
    parser = Mp4VttParser()
    init_path = Path(infile)
    parser.parseInit(init_path.read_bytes())
    segments_path = Path(inpath)
    ...
    for segment_path in segments_path.iterdir():
        ...
        for cue in _cues:
            ...
            cue.file = segment_path.name
            if len(cue.nestedCues) > 0:
                loop_nestedCues(cues, cue.nestedCues, index, segment_time)
            if cue.payload != '':
                cue.startTime += segment_time * index
                cue.endTime += segment_time * index
                cues.append(cue)
...
def main():
    ...
    parse(args)
    # python -m main --init-path "test/dashvtt_subtitle_WVTT_zh-TW/init.mp4" --segments-path "test/dashvtt_subtitle_WVTT_zh-TW"
